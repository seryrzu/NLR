...
#sigmoid2 <- function(x,y, alpha = 0.1) {  42.0 * exp(-7) / (10 * exp(-7) + exp(-(x * alpha + weights[y])) )}
sigmoid <- function(x,y, alpha = 0.1) { 0.03829904 / (0.00911882 +  + exp(-(x * alpha + weights[y]))) }
...
df = data.frame(x=runif(N,0,1000), y = as.factor(sample(c(1,2,3,4),size=100, prob=c(0.5,0.25,0.1,0.15), replace = TRUE)))
df$z = apply(df, function(row) { sigmoid(as.numeric(row[1]), 1) + rnorm(1)}, MARGIN=1)
nlin.fit <- nls(formula = z ~ a / (b+exp(-(alpha*x))), df, start=list(a=0.03829904,b=0.00911882,alpha=0.1))
summary(nlin.fit)
...
nlin.fit2 <- nls(formula = z ~ a / (b+exp(-(alpha*x+a2*y2+a3*y3+a4*y4))), a, start=list(a=0.03829904,b=0.00911882,alpha=0.1,a2=0.1,a3=2,a4=1))
summary(nlin.fit2)
...
cross.validation3 <- function(df, perc = 0.8) {
  ...
  test <- df[-train.ind,]
  nlin.fit2 <- nls(formula = z ~ a / (b+exp(-(alpha*x+a2*y2+a3*y3+a4*y4))), a, start=list(a=0.03829904,b=0.00911882,alpha=0.1,a2=0.1,a3=2,a4=1))
  predicted <- predict(nlin.fit2, test)
