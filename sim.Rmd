...
sigmoid <- function(x,y, alpha = 0.1) { 5 / (0.2 + exp(-(x * alpha + weights[y]))) }
...
cross.validation2 <- function(df, perc = 0.8) {
  ...
  train <- df[train.ind,]
  test <- df[-train.ind,]
  nlin.fit <- nls(formula = z ~ a / (b+exp(-(alpha*x + weights[y]))), train, start=list(a=0.1,b=0.3,alpha=0.1))
...
```{r}
nlin.fit <- nls(formula = z ~ a / (b+exp(-(alpha*x + weights[y]))), df, start=list(a=5, b=0.2, alpha=0.1))
summary(nlin.fit)
